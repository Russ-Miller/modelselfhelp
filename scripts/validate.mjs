...
const SCHEMA_FOR = { weaknesses: "weakness", papers: "paper", techniques: "technique", models: "model", claims: "claim" };
...
const checkEvidence = (file, ev) => {
  let supports = 0;
  for (const e of ev ?? []) { checkRef(file, ids.papers, e.paper, "paper"); if (e.stance === "supports") supports++; }
  return supports;
};
...
for (const w of cat.weaknesses) {
  const d = w.data;
  checkRef(w.file, groups, d.group, "group");
  for (const c of d.contexts ?? []) checkRef(w.file, contexts, c, "context");
  for (const t of d.techniques ?? []) checkRef(w.file, ids.techniques, t, "technique");
  for (const r of d.related ?? []) checkRef(w.file, ids.weaknesses, r, "weakness");
  if (d.parent) checkRef(w.file, ids.weaknesses, d.parent, "parent weakness");
  if (checkEvidence(w.file, d.evidence) === 0) problem(w.file, "needs at least one evidence entry with stance: supports");
}
for (const t of cat.techniques) {
  const d = t.data;
  for (const a of d.addresses ?? []) checkRef(t.file, ids.weaknesses, a, "weakness");
...
for (const c of cat.claims) {
  const d = c.data;
  checkRef(c.file, ids.weaknesses, d.weakness, "weakness");
...
// technique back-links: a weakness listing a technique must be in that technique's `addresses`
const addressed = new Map(cat.techniques.map((t) => [t.data.id, new Set(t.data.addresses ?? [])]));
for (const w of cat.weaknesses) for (const t of w.data.techniques ?? []) if (addressed.has(t) && !addressed.get(t).has(w.data.id)) problem(w.file, `technique ${t} does not list ${w.data.id} in addresses`);
