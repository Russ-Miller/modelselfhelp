...
import { claimsCiting, sourcesByRecency } from "@/lib/catalog";
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-neutral-500">
        ...
        the catalog, most recent first.
      </p>
      <table className="w-full text-sm">
        <thead className="text-left text-neutral-500">
          <tr><th className="py-1">Source</th><th>Created</th><th>Ingested</th><th>Cited by</th></tr>
        </thead>
        <tbody>
          {sources.map((s) => {
            const citing = claimsCiting(s.id);
            return (
              <tr key={s.id} className="border-t border-neutral-200 dark:border-neutral-800 align-top">
                <td className="py-2">
